...
customSchema.methods.weatherDescription = function(){
    ...
    var avg = this.avgtemp_f;

    var highLow = vsprintf("High/low will be %d°/%d°", [forecast.maxtemp, forecast.mintemp]);

    var prefixes = ["It'll be %s today. %s.%s %s", "Today will be %s. %s.%s %s", "Looks like we're in for %s weather today. %s.%s %s"];


    var temp = this._current_forecast.avgtemp_f;
    var weather = [];
...
customSchema.virtual('avgtemp_f').get(function () {
    ...
    this._past_forecasts.forEach(function(forecast){
        sum += parseFloat(forecast.avgtemp_f);
    });
