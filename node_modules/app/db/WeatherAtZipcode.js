...
customSchema.methods.updateWeather = function(callback){
    ...
    DailyForecast.forecastForLocation(this.lat, this.lon, function(err, forecast){
       if(err){
           ...
           //console.log("weather current forecast id : " + weather._current_forecast.id);
...
customSchema.methods.weatherDescription = function(){
    ...
    if (temps.length < hours.length){
        highLow = vsprintf("High/low today is %d°%s/%d°%s", [forecast.maxtemp, forecast.unitType, forecast.mintemp, forecast.unitType]);
    }
    else{
        highLow = vsprintf("Today's weather - 8AM: %d°%s, 1PM: %d°%s, 8PM: %d°%s", temp[0], forecast.unitType, temp[1], forecast.unitType, temp[2], forecast.unitType);
    }
    ...
    var msg = vsprintf("%s. %s", [highLow, this.precipDescription()]);
