...
var customSchema = new Schema({
        ...
        notification_time_utc: {type: String},
        notification_time_local: {type: String},
...
customSchema.statics.sendPushNotifications = function(users, title, message){
    ...
    if (onesignal_ids.length > 0){
        ...
        //console.log(onesignal_ids);

        OneSignal.sendPushNotification(onesignal_ids, title, message, function (err, response) {
            console.log("onesignal response[errors]: " + response["errors"]);
            err = err ? err : (response["errors"] ? response["errors"][0] : null);
            if (err) {
...
customSchema.methods.updateWithData = function(data, callback){
    ...
    mongoose.model("WeatherAtZipcode").findOne({"zipcode" : zipcode}).count(function(err, count){
        if (!err && count == 0) {
            mongoose.model("WeatherAtZipcode").createNewZipcode(data);
        }
        else if (err){
            callback(err);
            return;
    ...
    var updateFunc = function(){
        ...
        if (notification_time) {
            user.notification_time_local = notification_time;
            user.notification_time_utc = Globals.tzToUTCNotificationTime(notification_time, timezone);
        }

        if (onesignal_id && user.onesignal_ids.indexOf(onesignal_id) == -1){
            user.onesignal_ids = [onesignal_id];
        }

        if (push_token){
            var currToken = user.push_tokens.find(function(token){
                return token === push_token;
            });

            if (os && !currToken){
                user.push_tokens = [{os: os, push_token: push_token}];
            }
        }
...
customSchema.methods.validateSecretKey = function(secret_key, callback){
    if (secret_key === this.secret_key){
        console.log("secret_keys are equal stupid.");
