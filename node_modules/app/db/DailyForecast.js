...
customSchema.statics.forecastForLocation = function(lat, lon, callback){
    var url = "https://api.darksky.net/forecast/" + process.env.APIXU_API_KEY + "/" + lat + "/" + lon;
    ...
    Globals.requestUrl(url, function(err, json){
        ...
        if (err){
            ...
            var maxtemp = null, mintemp = null, avgtemp = 0;
            ...
            for (var i = 0; i < hourly.length; i++){
                ...
                var apparentTemp = hour["apparentTemperature"];
                ...
                if (hour["precipProbability"] >= Globals.MIN_PRECIP_PROBABILITY && hour["precipType"] == "rain" && rainStartTime == -1){
                    rainStartTime = i;
                }
                if (hour["precipProbability"] >= Globals.MIN_PRECIP_PROBABILITY && hour["precipType"] == "snow" && snowStartTime == -1){
                    snowStartTime = i;
                }

                //var hourlyForecast = HourlyForecast.withJson(hour);
                //hourlyForecast.save();
                //dailyForecast._hourly_forecasts.push(hourlyForecast._id);
            }

            avgtemp /= hourly.length;
